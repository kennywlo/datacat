...
drop table if exists ContainerSearch;
...
drop table if exists DatasetMetaname ;
...
alter table if exists DatasetVersion drop constraint FK_DSV_MasterLocation;
drop table  if exists VerDatasetLocation ;
drop table  if exists DatasetSite ;

alter table if exists VerDataset drop constraint FK_VDS_LatestVersion;
...
create table DatasetDependency (
    DatasetDependency integer GENERATED BY DEFAULT AS SEQUENCE dataset_seq,
    Dependency      integer,
    DependencyGroup integer,
    DependencyName  varchar(255) not null,
    Dependent       integer,
    DependentGroup  integer,
